...
/*
snprintf(strbuf, sizeof(strbuf) - 1,
"cat /sys/class/net/%s/phy80211/subsystem >/dev/null 2>/dev/null", iface);
system(strbuf);
*/
	/* IPW2200 detection */
	memset(strbuf, 0, sizeof(strbuf));
	...
	if (strlen(iface) == 5 && memcmp(iface, "wlan", 4) == 0) {
		memset(strbuf, 0, sizeof(strbuf));
		snprintf(strbuf, sizeof(strbuf) - 1,
				"wlancfg show %s 2>/dev/null | grep p2CnfWEPFlags >/dev/null", iface);

		if (system(strbuf) == 0) {
			if (uname(&checklinuxversion) >= 0) {
				/* uname succeeded */
				if (strncmp(checklinuxversion.release, "2.6.", 4) == 0
						&& strncasecmp(checklinuxversion.sysname, "linux", 5)
								== 0) {
					/* Linux kernel 2.6 */
		...
		memset(strbuf, 0, sizeof(strbuf));
		snprintf(strbuf, sizeof(strbuf) - 1,
				"iwpriv %s 2>/dev/null | grep antsel_rx >/dev/null", iface);
		...
		memset(strbuf, 0, sizeof(strbuf));
		snprintf(strbuf, sizeof(strbuf) - 1,
				"iwpriv %s 2>/dev/null | grep GetAcx111Info >/dev/null", iface);
			...
			memcmp(iface, "ath", 3) == 0)
	{
		...
		snprintf(strbuf, sizeof(strbuf) - 1, "/proc/sys/net/%s/%%parent", iface);
		...
		if (f != NULL) {
			...
			memset(strbuf, 0, sizeof(strbuf));
			snprintf(strbuf, sizeof(strbuf) - 1,
					"sysctl -w dev.%s.rawdev=1 >/dev/null 2>/dev/null", iface);
	...
	if (memcmp(iface, "eth", 3) == 0) {
		...
		memset(strbuf, 0, sizeof(strbuf));
		snprintf(strbuf, sizeof(strbuf) - 1,
				"iwpriv %s 2>/dev/null | grep get_scan_times >/dev/null", iface);
	...
	if (dev->drivertype == DT_IPW2200) {
		snprintf(r_file, sizeof(r_file), "/sys/class/net/%s/device/rtap_iface", iface);
...
//printf("start do_linux_open %ld\n", time(NULL));
	if (do_linux_open(wi, iface)) {
		do_free(wi);
		return NULL;
	}
//printf("end   do_linux_open %ld\n", time(NULL));
	...
	if (linux_apm == 1) {
		...
		if (battery_data != NULL) {
			...
			ret = sscanf(battery_data, "%*s %*d.%*d %*x %x %x %x %*d%% %d %s\n",
					&ac, &charging, &flag, &batteryTime, units);
			if (!ret)
				return 0;
		...
		while (batteries != NULL
			...
			if (info != NULL) {
				while (fgets(buf, sizeof(buf), info) != NULL)
					if (sscanf(buf, "last full capacity:      %d mWh",
							&batt_full_capacity[batno]) == 1)
						continue;
				fclose(info);
			}
