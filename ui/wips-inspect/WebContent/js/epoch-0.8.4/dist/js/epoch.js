...
Epoch.Time.Plot = (function(superClass) {
  ...
  Plot.prototype._startTransition = function() {
	...
	// KINOW [ 1 sec ] -> [ speed: msec ]
    return this.animation.interval = setInterval(this.animationCallback, this.options.speed / this.options.fps);
  };
  ...
  Bar.prototype.draw = function(delta) {
    ...
    for (j = 0, len = ref1.length; j < len; j++) {
      ...
      while ((--i >= iBoundry) && (--k >= 0)) {
		...
		// KINOW - [ w-2 ] -> [ 0.4 ]
        args = [ex + 1, y(ey + ey0), /*w - 2*/0.4, this.innerHeight() - y(ey) + 0.5 * this.pixelRatio];
