...
page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%><%
	...
	*/
			...
			*/
			...
			if (tagCount > 0) {
				...
				for (int t=0; t<tagCount; t++) {		
					...
					tag.setId(tag_id[index].trim());
					tag.setLen(tag_length[index].trim());
					tag.setData(tag_value[index].trim());
			...
			if (!StringUtil.isNull(enable[i])) {
				ac.setEnable(enable[i].trim().toLowerCase());
			}
			if (!StringUtil.isNull(desc[i])) {
				ac.setDesc(desc[i].trim());
			}
			if (!StringUtil.isNull(dwell[i])) {
				ac.setDwell(dwell[i].trim());
			}
			if (!StringUtil.isNull(channel[i])) {
				ac.setChannel(channel[i].trim());
			}
			if (!StringUtil.isNull(frame[i])) {
				String type    = String.valueOf(Byte.parseByte(frame[i].substring(0,1), 16));
				String subtype = String.valueOf(Byte.parseByte(frame[i].substring(1,2), 16));
				ac.setType(type);
				ac.setSubtype(subtype);
			}
			...
			if ("addr1".equals(address[i])) {
				ac.setAddrCount("1");
				ac.setDa(addr1_da[i].trim().toLowerCase());
			} else if ("addr2".equals(address[i])) {
				ac.setAddrCount("2");
				ac.setDa(addr2_da[i].trim().toLowerCase());
				ac.setSa(addr2_sa[i].trim().toLowerCase());
			} else if ("addr3".equals(address[i])) {
				ac.setAddrCount("3");
				ac.setBssid(addr3_bssid[i].trim().toLowerCase());
				ac.setDa(addr3_da[i].trim().toLowerCase());
				ac.setSa(addr3_sa[i].trim().toLowerCase());
			} else if ("addr4".equals(address[i])) {
				ac.setAddrCount("4");
				ac.setRa(addr4_ra[i].trim().toLowerCase());
				ac.setTa(addr4_ta[i].trim().toLowerCase());
				ac.setDa(addr4_da[i].trim().toLowerCase());
				ac.setSa(addr4_sa[i].trim().toLowerCase());
			} else if ("apsta".equals(address[i])) {
				ac.setAddrCount("0");
				ac.setAp(ap[i].trim().toLowerCase());
				ac.setSta(sta[i].trim().toLowerCase());
			} else if ("any_addr".equals(address[i])) {
				ac.setAddrCount("0");
				ac.setAnyAddr(any_addr[i].trim().toLowerCase());
			}
	...
	try {
		...
		Config c = new Config();
		c.setId(configId);
		c.setName(HttpGet.get(request, "config_name").trim());
		if (shooterXmlFilename != null) {
		...
		e.printStackTrace();
		/*
		* When failed insert or update, delete current config
		*/
